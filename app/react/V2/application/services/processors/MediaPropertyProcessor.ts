...
export class MediaPropertyProcessor implements PropertyTypeProcessor {
    ...
    async processBatch(
        ...
        properties.forEach(property => {
            try {
                ...
                const timelines = context.options.editionMode && property.timelines
                ...
                const formattedProperty: FormattedProperty = {
                    ...property,
                    values: mediaFiles,
                    timelines: context.options.editionMode ? timelines : undefined,
    ...
    private processMediaFiles(files: any[], context: ProcessingContext): PropertyValue[] {
    ...
    private processTimelines(timelines: any[], context: ProcessingContext): PropertyValue[] {
    ...
    private calculateTotalSeconds(timeline: any): number {
        const hours = parseInt(timeline.timeHours) || 0;
        const minutes = parseInt(timeline.timeMinutes) || 0;
        const seconds = parseInt(timeline.timeSeconds) || 0;
